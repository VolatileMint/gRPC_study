...
func main() {
	...
	conn, err := grpc.Dial(
		...
		// grpc.WithChainUnaryInterceptor( // 複数インターセプタの導入
...
func HelloServerStream() {
	fmt.Println("Please enret your name.")
	scanner.Scan()
	name := scanner.Text()
	...
	for {
		res, err := stream.Recv()
		if errors.Is(err, io.EOF) {
			fmt.Println("all the responses have already recieved.")
			break
		}
		if err != nil {
			fmt.Println(err)
